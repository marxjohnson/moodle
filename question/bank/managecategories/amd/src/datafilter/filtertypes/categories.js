...
export default class extends GenericFilter {
    ...
    async addSubcategoryCheckbox(checked = false) {
        ...
        const {html} = await Templates.renderForPromise('qbank_managecategories/subcategory_checkbox', {checked: checked});
        filterValueNode.insertAdjacentHTML('beforeend', html);
    }

    get filterOptions() {
        return {
            includesubcategories: this.getFilterValueNode().querySelector(this.SELECTORS.includeSubcategories).checked
        };
    }
