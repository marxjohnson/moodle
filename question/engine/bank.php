...
class question_finder implements data_source_interface {
    ...
    public function load_many_for_cache(array $questionids) {
        ...
        foreach ($questionids as $id) {
            ...
            // $plugindata is in the format $plugindata[$pluginname][$questionid][$item] = [$key => $value], so we end up with
            // $questiondata[$id]->qbank[$pluginname] = [$item => [$key => $value]] for each datum.
