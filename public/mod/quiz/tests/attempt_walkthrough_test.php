...
final class attempt_walkthrough_test extends \advanced_testcase {
    ...
    /**
     * An attempt that contains automatically-gradable questions can be graded asynchronously.
     */
    public function test_quiz_attempt_walkthrough_automatically_gradable_questions(): void {
        $this->resetAfterTest();
        ...
        // Create a quiz with a true/false question.
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        ...
        $this->expectOutputRegex(
            "~^Grading attempt for user ID {$user1->id} for quiz {$quiz->name} on course {$course->shortname}\n~"
        );
