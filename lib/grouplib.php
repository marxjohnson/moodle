...
function groups_get_all_groups($courseid, $userid=0, $groupingid=0, $fields='g.*', $withmembers=false, $participationonly = false) {
    ...
    if (!$viewhidden) {
        ...
        [$insql, $inparams] = $DB->get_in_or_equal([\core_group\visibility::MEMBERS, \core_group\visibility::OWN]);
        $visibilitywhere = "AND (g.visibility = ? OR (g.visibility $insql AND gm.id IS NOT NULL))";
        $params = array_merge(
            $userids,
            $params,
            [\core_group\visibility::ALL],
            $inparams
        );
    }
    ...
    if (!$viewhidden) {
        ...
        $gmvisibilityparams = [
            \core_group\visibility::ALL,
            \core_group\visibility::MEMBERS,
            $USER->id,
            \core_group\visibility::OWN,
            $USER->id
        ];
    }
...
function groups_get_my_groups() {
    ...
    $visibilitywhere = '';
    if (!$viewhidden) {
        $params['novisibility'] = \core_group\visibility::NONE;
