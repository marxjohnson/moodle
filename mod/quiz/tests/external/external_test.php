...
class external_test extends externallib_advanced_testcase {
    ...
    /**
     * Test get_user_attempts when the attempt is in 'notstarted' state.
     *
     * @todo Remove in Moodle 6.0 as part of MDL-80956 final deprecations.
     * @covers \mod_quiz_external::get_user_attempts
     */
    public function test_get_user_attempts_notstarted(): void {
        ...
        $this->assertEquals(quiz_attempt::IN_PROGRESS, $result['attempts'][0]['state']); // State is returned as in progress.
    }
    ...
    public function test_get_attempt_data(): void {
        ...
        $result = mod_quiz_external::get_quiz_attempt_review($attempt->id, 1);
        $result = external_api::clean_returnvalue(mod_quiz_external::get_quiz_attempt_review_returns(), $result);
    ...
    public function test_get_attempt_review_submitted(): void {
        ...
        [, , , $attempt, $attemptobj, ] = $this->create_quiz_with_questions(true);
        // Submit the attempt but do not finish it.
        // Process some responses from the student.
        $tosubmit = [1 => ['answer' => '3.14']];
        $attemptobj->process_submitted_actions(time(), false, $tosubmit);
     ...
     * Test get_attempt_review
     *
     * @covers \mod_quiz_external::get_quiz_attempt_review
     */
    public function test_get_quiz_attempt_review(): void {
        ...
        $result = mod_quiz_external::get_quiz_attempt_review($attempt->id);
        $result = external_api::clean_returnvalue(mod_quiz_external::get_quiz_attempt_review_returns(), $result);
        ...
        // Only first page.
        $result = mod_quiz_external::get_quiz_attempt_review($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_quiz_external::get_quiz_attempt_review_returns(), $result);
    ...
    /**
     * Test get_quiz_attempt_review with extra grades
     */
    ...
    /**
     * Test get_quiz_attempt_review for an attempt in 'submitted' state.
     *
     * @covers \mod_quiz_external::get_quiz_attempt_review
     */
