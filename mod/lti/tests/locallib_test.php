...
-----END PUBLIC KEY-----';
    ...
    /**
     * Test that deprecated lti_get_ims_role() outputs a debugging message.
     *
     * @covers ::lti_get_ims_role()
     */
    public function test_lti_get_ims_role() {
        $this->resetAfterTest();

        $course = $this->getDataGenerator()->create_course();
        $user = get_admin();

        lti_get_ims_role($user, 0, $course->id, true);
        $this->assertDebuggingCalled('lti_get_ims_role() is deprecated. Please use mod_lti\helper::get_ims_role() instead.');
    }
