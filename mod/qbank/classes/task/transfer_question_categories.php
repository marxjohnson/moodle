...
class transfer_question_categories extends adhoc_task {
    ...
    protected function question_category_is_empty(int $categoryid): bool {
        ...
        $empty = !$DB->record_exists('question_bank_entries', ['questioncategoryid' => $categoryid]);

        if ($empty) {
            // If this category is empty, recursively check child categories.
            $children = $DB->get_fieldset('question_categories', 'id', ['parent' => $categoryid]);
            if ($children) {
                foreach ($children as $childid) {
                    $empty = $empty && $this->question_category_is_empty($childid);
                    if (!$empty) {
